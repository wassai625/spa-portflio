...
const ApiContextProvider = (props) => {
  ...
  useEffect(() => {
    const getMyProfile = async () => {
      try {
        const resmy = await axios.get(
          "http://52.194.180.73:8000/api/user/profile",
          {
            headers: {
        ...
        const res = await axios.get(
          "http://52.194.180.73:8000/api/user/approval",
          {
            headers: {
              Authorization: `Token ${token}`,
            },
    ...
    const getProfile = async () => {
      try {
        const res = await axios.get("http://52.194.180.73:8000/api/user/profile", {
          headers: {
            Authorization: `Token ${token}`,
          },
        });
    ...
    const getInbox = async () => {
      try {
        const res = await axios.get("http://52.194.180.73:8000/api/dm/inbox", {
          headers: {
            Authorization: `Token ${token}`,
          },
        });
  ...
  const deleteProfile = async () => {
    try {
      await axios.delete(
        `http://52.194.180.73:8000/api/user/profile${profile.id}/`,
        {
          headers: {
            "Content-Type": "application/json",
            Authorization: `Token ${token}`,
  ...
  const sendDMCont = async (uploadDM) => {
    try {
      await axios.post(`http://52.194.180.73:8000/api/dm/message`, uploadDM, {
        headers: {
          "Content-Type": "application/json",
          Authorization: `Token ${token}`,
        },
  ...
  const changeApprovalRequest = async (uploadDataAsk, ask) => {
    try {
      const res = await axios.put(
        `http://52.194.180.73:8000/api/user/approval${ask.id}/`,
        uploadDataAsk,
        {
          headers: {
            "Content-Type": "application/json",
      ...
      !resp[0]
        ...
        : await axios.put(
            `http://52.194.180.73:8000/api/user/approval${resp[0].id}/`,
            newDataAskPut,
            {
              headers: {
                "Content-Type": "application/json",
  ...
  return (
    <ApiContext.Provider
      value={{
        ...
        cover,setCover,
        askList,askListFull,
        inbox,newRequestFriend,
        createProfile,editProfile,
        deleteProfile,
        changeApprovalRequest,
        sendDMCont,editedProfile,
        setEditedProfile,
      }}
    >
      {props.children}
